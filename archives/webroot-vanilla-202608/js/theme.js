const CasTheme = {
  ...
  applySystemBarIcons(barDark) {
    ...
    if (trySet(window.$CertBridge)) return;
  ...
  setMonet(on) {
    ...
    if (typeof CasUi !== "undefined")
      CasUi.toast(on ? "已开启莫奈取色" : "已关闭莫奈取色");
  },
  ...
  apply(mode) {
    ...
    if (typeof CasUi !== "undefined") {
      CasUi.renderChips("themeChips", this.PRESETS, selected, (id) =>
        this.setMode(id),
      );
    }
  ...
  setMode(mode) {
    ...
    this.apply(next);
    if (typeof CasUi !== "undefined")
      CasUi.toast(`已切换为${this.LABELS[next]}`);
  },
