...
- 为 APEX 与 system 信任库分别建立临时挂载层，并按目标路径设置 SELinux 后再绑定到各命名空间
- 修复只读重挂载失败时错误撤销已成功绑定，导致系统证书可见但抓包 App 仍检测未安装、TLS 失败的问题
- 开机注入结果写入运行时状态缓存；WebUI 与模块简介只读缓存，避免重复全量命名空间探测，并取消 WebUI 后台轮询
- 修复开机服务覆盖模块属性导致简介反复回到「检测中」的问题
