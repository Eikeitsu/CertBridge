...
- **证书开关**：关/开共用一套不变量——关只改 conf 并快照本地证（不删 sources）；开认 sources / stash / 生效集 / builtin，App 同步仅作刷新。WebUI 隔离 ns 可经 init 探测导入；热更新保留 `certs/sources` 与 `source-stash`；写 conf 读回校验、契约行合并识别，避免误报保存失败或回弹
