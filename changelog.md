...
- **优化免重启热更新的生效速度**：暂存目录复制完成后立即清除更新标记并执行 `hotinstall.sh`，不再等待后台残留清理窗口结束；对 InstallX 等第三方安装器的延迟写回仍由后台继续清理，并增加同模块互斥锁，避免重复拉起多个收尾作业

- 非首次更新支持免重启热更新：`system/` / sepolicy / zygisk 未变时可热切换并重跑 `post-fs-data`+`service`；变更上述路径或首次安装仍需重启
- 修复热更新经常不生效（`modules_update` 残留、模块目录留下 `update` 标记）：目录切换作业改用 `setsid` 脱离安装器会话，不再被安装结束时连带杀掉；等待方式从固定 5 秒改为等暂存目录稳定，避免与管理器写 `update` 标记赛跑，并在覆盖后持续清理残留；覆盖前校验暂存关键文件齐全，中断的暂存不会盖掉正在用的模块；过程记入 `data/hot-update.log`
- 提高第三方安装器（InstallX 等）下热更新的成功率：这类安装器 `touch update` 的时机比管理器晚得多，原先 20 秒的清理窗口常常是我们先退场、它后写回标记，用户就又看到「需重启」。现在窗口放宽到 120 秒并记下清理次数；收尾作业启动即写一行 `start` 日志，`data/hot-update.log` 里有没有这行可以直接区分「没跑起来」和「跑了但失败」
