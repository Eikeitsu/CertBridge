...
- 恢复经过验证的系统 CA 基线方案，始终以“完整系统基线 + 模块证书”生成挂载内容
- 安装时至少捕获 10 张系统 CA 才允许继续，基线缺失时直接中止安装
- Android 14+ APEX 与传统 system 路径均从同一系统基线生成独立 tmpfs
- 保留 PID 1、Zygote 与系统设置进程注入日志，便于定位不同 Root 环境的挂载结果
