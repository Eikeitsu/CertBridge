...
| 被抓包 App 断网、证书错误                 | 对**目标 App** 开了 umount / 卸载模块                                       |
...
开机 `service` **只**补齐 PID 1、Zygote、Settings（**不强注** Reqable / ProxyPin，以免抵消「卸载模块」），**不会**扫全机应用（避免卡住）。新启动的 App 一般继承 Zygote 挂载；**已在运行**的目标 App 请强停再开。热挂载才会尝试覆盖更多当前可访问的活动命名空间；WebUI 可能显示部分未覆盖，可强停后重挂或重启。
