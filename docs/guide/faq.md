...
## 为什么刚重启显示「异常 / 注入失败」，过一会又正常？

开机完成瞬间 Zygote / 命名空间偶发未就绪，旧版会立刻把校验结果写成失败并缓存。现已：

1. `service` 阶段对校验做退避重试
2. 约 45 秒后若仍是失败缓存会再测一次并自愈写回
3. WebUI「刷新复核」强制 `status --live` 实测

若抓包已正常而简介仍异常，点一次刷新复核即可；仍失败再查日志。
...
4. 若简介为「⚠️异常」，先看 **WebUI 概览** 上的诊断原因与建议，或 **Action** 中的「注入错误 / 建议」行；再打开日志页搜索 `inject:`、`generation:`
5. 日志写「实时证书集合生成失败」「开机证书注入失败」时，查看实时源数量与具体失败点

若以上都正常仍 TLS 失败 / 断网，优先怀疑**证书本身不一致**（见下一节）。若只有「单应用白名单」断网、「抓全部」正常，见 [白名单单应用断网](#whitelist-disconnect)。

## Reqable 显示「根证书未安装」，或抓包一开就断网？

先查 **Root 隐藏是否卸掉了证书挂载**：

| 现象                                      | 常见原因                                                                    |
| ----------------------------------------- | --------------------------------------------------------------------------- |
| Reqable / ProxyPin 里显示「根证书未安装」 | 对该抓包软件开了 **卸载模块 / Umount**、DenyList+umount、APatch「排除修改」 |
| 被抓包 App 断网、证书错误                 | 对**目标 App** 开了同类 umount / 卸载模块                                   |

证书桥的 CA 靠 bind 挂在 cacerts 上；对某进程开启卸载模块后，该进程看不到模块挂载，证书层也会消失。

**处理：** 对 Reqable / ProxyPin **以及**本次被抓包的目标 App，在 KernelSU / Magisk / APatch 管理器里**关闭**「卸载模块 / 排除修改」等隐藏；仅对不参与抓包、需要躲检测的其它 App 开启。详见 [挂载隐藏 · 抓包必读](/guide/hide#抓包必读不要对抓包链路开卸载模块)。
...
2. **临时 · 热挂载**（已安装该组件时）— 用户区或 `/sdcard/Documents/cacerts`；重启后失效
...
因此默认「完整兼容」模式**不依赖** Magic Mount、也不长期放整库到 `system/`。若启用轻量模式后证书总数骤降，请改回完整兼容并重启；KernelSU 用户请确认挂载/元模块提供的是文件叠层而非整目录替换。

## 轻量 Magic 模式一定要装元模块吗？

- **Magisk**：一般**不需要**，自带 Magic Mount。
- **KernelSU**：取决于管理器是否对 `system/` 做正确文件叠层；很多人依赖挂载相关**元模块**。若启用轻量后系统 CA 数量骤降，立刻改回完整兼容并重启。
- **APatch**：视版本而定，异常时用完整兼容。

两种模式的对照表见 [配置说明 · 挂载模式](/guide/config#挂载模式)。

## 注入失败时 WebUI 显示什么？

模块会把失败原因写成 **可读短句 + 建议**（不再只有「请查看日志」）：

- **WebUI 概览**：诊断卡片 +「查看日志」
- **Action**：`注入错误:` 与 `建议:` 行
- **模块简介**：`[⚠️异常|…]` 后附带同样说明

常见原因包括：证书集生成失败、临时层创建失败、bind / 命名空间注入失败、轻量叠层未生效、校验未通过等。详细对照见 [WebUI 使用说明 · 注入失败诊断](/guide/webui#注入失败诊断)。
...
自定义安装时若跳过热挂载，不会保留 `bin/hot_mount.sh`，WebUI 与 Action 也会隐藏或提示未安装。请重新刷入并勾选，或选默认安装。
