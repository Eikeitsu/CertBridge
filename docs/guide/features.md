...
默认安装会检测已安装抓包 App 并导入 CA，同时安装 WebUI、热挂载与挂载隐藏协助（`hide_allow` 默认关闭）；自定义安装可逐项选择 Reqable、ProxyPin、WebUI、免重启热挂载或挂载隐藏协助（勾选隐藏后 `hide_allow` 默认开启）。若检测到 **HttpCanary** 或 **ADGuard**，会依次询问是否导入为自定义证书。
