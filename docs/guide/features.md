...
默认安装会检测已安装抓包 App 并导入 CA，同时安装 WebUI 与热挂载（**不**安装挂载隐藏协助）；自定义安装可逐项选择 Reqable、ProxyPin、WebUI、免重启热挂载或挂载隐藏协助。若检测到 **HttpCanary** 或 **ADGuard**，会依次询问是否导入为自定义证书。
...
| 挂载隐藏协助（可选）   | 自定义安装可选；WebUI「隐藏」页开关控制是否登记 SuSFS try_umount；见 [挂载隐藏说明](/guide/hide)                                                              |
| Action 只读仪表盘      | 版本 / 状态 / 注入诊断 / 热挂载摘要 / 日志路径（约 10s；改配置请用 WebUI）                                                                                    |
| WebUI（可选）          | React：概览、证书、日志、更多；安装隐藏组件后多「隐藏」页；见 [WebUI 使用说明](/guide/webui)                                                                  |
