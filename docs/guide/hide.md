...
配置项 `force_bind_capture`（WebUI「隐藏」页「强注抓包 App」）：
...
配置项 `late_inject`（WebUI「隐藏」页「开机后晚注入」）：
...
证书主路径仍是开机 **boot** 注入。多数机型关着即可；仅当部分 App 重启后仍读不到 CA 时再开。部分环境晚注入可能加重 Found KSU 类误伤。
