...
强注、晚注入、开机痕迹相关开关收在此子页，默认无需改动。

#### 强注抓包 App（旧行为）

配置项 `force_bind_capture`：

| 值            | 行为                                                                                                          |
| ------------- | ------------------------------------------------------------------------------------------------------------- |
| **0（默认）** | 命名空间注入**不**强注 Reqable / ProxyPin，尊重 KSU「卸载模块」                                               |
| **1**         | **立刻**对正在运行的抓包 App 后台补绑；之后开机/命名空间注入也会再 bind。开着卸载模块也可能显示「证书已安装」 |

关闭强注后请**强停**抓包 App 再开以去掉已绑挂载（不必整机重启）。仅建议抓包调试时短期开启。
...
| 值            | 行为                                                                                                    |
| ------------- | ------------------------------------------------------------------------------------------------------- |
| **0（默认）** | `service.sh` 在 `boot_completed` 后**只收尾写状态**，不再跑 `namespaces` 注入（仅 boot 注入、痕迹更少） |
| **1**         | `boot_completed` 后再补应用侧命名空间注入；开启开关时也会**立刻后台补一次**。难机更稳，但对进程介入更多 |

证书主路径仍是开机 **boot** 注入。多数机型关着即可；仅当部分 App 重启后仍读不到 CA 时再开。

#### 进一步减轻开机痕迹

| 配置               | 默认 | 说明                                                             |
| ------------------ | ---- | ---------------------------------------------------------------- |
| `boot_bind_zygote` | `0`  | `1`=开机 nsenter zygote（兼容）；默认仅 bind init                |
| `boot_multi_apex`  | `0`  | `1`=完整双模式目标；默认 14+ 仅主 APEX（跳过 `@版本` 与 system） |
| `service_probe`    | `0`  | 仅 `late_inject=1` 时生效：`1`=退避与延迟 heal                   |

改 `boot_*` 后需重启。CLI：`cb set boot_bind_zygote 0` 等。默认保持兼容优先，与「完整兼容 / 轻量 Magic」双模式产品设计不冲突。
...
| 事实        | 说明                                                                                     |
| ----------- | ---------------------------------------------------------------------------------------- |
| 路径迁移    | 默认临时层在 `/dev/.fs*` 是为避开对 `/data/local/tmp` 的关键词扫描，**不能**让 bind 消失 |
