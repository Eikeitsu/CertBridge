...
无助手时：mountinfo 上的 bind 仍可能被检测发现。页面「隐藏助手」会并列展示本机可用的 SuSFS / ksud / NoHello / ZygiskNext 等（可同时生效，无互斥优先级）；隐藏协助由本模块直接调用 `ksud kernel umount` / `ksu_susfs add_try_umount` 登记路径。
