...
升级会尽量保留 `certs.conf`、自定义证书、`certs/sources`、关断快照（`source-stash`）与安装档案；组件是否安装以本次刷入选择为准。

## 热更新说明

支持免重启热更新（管理器 / WebUI 安装模块时）。外部短时文件在 `/data/adb/certbridge/`；失败会回退到需重启的标准更新。热更新后 **WebUI / CLI 脚本立即生效**，可直接验证开关；本地证书目录会随升级保留，不必为验证开关而重启。
