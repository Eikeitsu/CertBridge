...
证书桥 WebUI 在模块管理器中打开（KernelSU 管理器、MMRL、WebUI-X 等）。界面为 **React** 单页应用，底部导航：**概览 · 证书 · 日志 · 更多**；安装了**挂载隐藏协助**和/或 **Zygisk 挂载痕迹过滤**时会多一页 **隐藏**（默认安装会装上隐藏协助，故通常可见隐藏页；Zygisk 过滤仅自定义安装可选）。
...
| Zygisk 过滤开关 | `zn_hide_allow`；改后需重启相关 App；未安装时提示需自定义重刷                             |
