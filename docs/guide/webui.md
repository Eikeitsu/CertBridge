...
| SuSFS 开关     | `hide_allow`：默认安装为关、自定义勾选隐藏为开；关闭后不登记 try_umount、不写隐藏状态文件 |
| Zygisk 过滤开关 | `zn_hide_allow`：仅组件已安装时显示；改后需重启相关 App                                 |
| 简介           | 换路径不能替代 umount；可跳转文档站                                                       |
| 挂载与隐藏实况 | Root 方案、挂载模式、临时层路径、检测到的隐藏助手、SuSFS try_umount、Zygisk 过滤状态      |
