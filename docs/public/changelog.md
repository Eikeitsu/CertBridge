...
- 修复开机后状态卡住「注入中」、WebUI 误报注入失败：不再在 post-fs 过早判定失败，并取消对全部进程命名空间的扫描（可导致 service 挂起）
- 放宽 SELinux 上下文比对（兼容带 MCS 类别的机型），避免误拒证书层准备
- 修正注入后过度校验：绑定已成功时，内容/归属检查失败只记日志、不再 umount 回滚；并取消 bind 后的 remount,ro（部分机型上会导致「已安装」但抓包仍断网）
- 优化音量键选择：每轮独立计时，降低 Action 菜单漏键 / 需连按才响应的问题
