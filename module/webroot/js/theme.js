const CasTheme = {
  KEY: 'cas_theme_mode',
  MONET_KEY: 'cas_monet',
  LAYOUT_KEY: 'cas_layout',
  COMPACT_KEY: 'cas_compact',
  LABELS: {
    light: '浅色模式',
    dark: '深色模式',
    system: '跟随系统'
  ...
  setMonet(on) {
    ...
    this.applyMonet(on);
    this.syncStatusBar();
    if (typeof CasUi !== 'undefined') CasUi.toast(on ? '已开启莫奈取色' : '已关闭莫奈取色');
  },
  ...
  applyLayout(layout) {
    ...
    if (typeof CasNav !== 'undefined') CasNav.apply(mode);
  },
  ...
  setLayout(layout) {
    ...
    if (typeof CasUi !== 'undefined') {
      CasUi.toast(mode === 'dock' ? '已开启悬浮分页' : '已切换为经典单页');
    }
  },
  ...
  setCompact(on) {
    ...
    if (typeof CasUi !== 'undefined') {
      CasUi.toast(on ? '已开启卡片紧凑' : '已关闭卡片紧凑');
    }
  },
  ...
  apply(mode) {
    ...
    this.syncStatusBar();
    if (typeof CasUi !== 'undefined') CasUi.syncTopbarSpacer();
    ...
    if (desc) {
      ...
      desc.textContent =
        selected === 'system' ? `${label}（当前${resolved === 'dark' ? '深色' : '浅色'}）` : label;
    }

    if (typeof CasUi !== 'undefined') {
      CasUi.renderChips('themeChips', this.PRESETS, selected, (id) => this.setMode(id));
    }
  },
  ...
  setMode(mode) {
    ...
    this.apply(next);
    if (typeof CasUi !== 'undefined') CasUi.toast(`已切换为${this.LABELS[next]}`);
  },
