...
bool find_libc(dev_t *dev, ino_t *inode) {
  ...
  while (fgets(line, sizeof(line), fp)) {
    if (!std::strstr(line, "libc.so"))
      continue;
    if (!std::strstr(line, "r-xp") && !std::strstr(line, "r--p"))
      continue;
    unsigned int maj = 0, min = 0;
    ino_t ino = 0;
    if (sscanf(line, "%*s %*s %*s %x:%x %lu", &maj, &min, &ino) < 3)
      continue;
    *dev = makedev(maj, min);
    *inode = ino;
    ok = true;
...
ssize_t hooked_readlink(const char *pathname, char *buf, size_t bufsiz) {
  ssize_t n =
      orig_readlink ? orig_readlink(pathname, buf, bufsiz) : ::readlink(pathname, buf, bufsiz);
  if (!g_enabled)
    return n;
