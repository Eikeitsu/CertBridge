...
#
# Inputs: .build/release/CertBridge_*.zip (after stamp + package), module/module.prop
set -euo pipefail
...
chmod +x scripts/git-push-tree.sh
...
pick_zip() {
  ...
  local src=""
  shopt -s nullglob
  for z in .build/release/$pattern; do
    src="$z"
  done
  shopt -u nullglob
  if [ -z "$src" ] || [ ! -f "$src" ]; then
    echo "missing zip matching .build/release/$pattern" >&2
    exit 1
  fi
  cp "$src" "$STAGE/$dest"
...
if ls .build/release/CertBridge_*_lite.zip >/dev/null 2>&1; then
...
FAT_SRC=""
shopt -s nullglob
for z in .build/release/CertBridge_*.zip; do
  case "$z" in
    *_lite.zip) continue ;;
...
scripts/git-push-tree.sh ci-dist "$STAGE" \
