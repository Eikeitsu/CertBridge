...

{
  ...
  echo "cbx509:"
  hash_tree tooling/cbx509/src
  for f in tooling/scripts/build-cbx509.mjs tooling/scripts/build-web.mjs tooling/scripts/package-module.mjs; do
    if [ -f "$f" ]; then sha256sum "$f"; else echo "MISSING $f"; fi
  done
  ...
  if [ -d native/zygisk_hide ]; then
    ...
    if [ -f tooling/scripts/build-zygisk-hide.mjs ]; then
      sha256sum tooling/scripts/build-zygisk-hide.mjs
    fi
  fi
} >"$TMP"
